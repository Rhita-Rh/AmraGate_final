...
export default {
  ...
  try {
    ...
    if (docSnap.exists()) {
      ...
      this.updateElapsedTimes(); // Call here to update elapsed times
    }
  } catch (error) {
...
calculateElapsedTime(dateString) {
  ...
  const days = Math.floor(seconds / (3600 * 24)); // Convert to days

  if (minutes < 1) {
    return 'Just now'; // For moments ago (less than 1 minute)
  }
...
mounted() {
  this.updateElapsedTimes(); // Initial call
